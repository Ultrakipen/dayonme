...
const HomeScreen: React.FC<HomeScreenProps> = ({ navigation }) => {
    ...
    // 새 글 작성 완료 후 파라미터 처리 및 글로벌 이벤트 감지
    useFocusEffect(
        useCallback(() => {
            // TODO: React Native 0.80 호환성 - global 객체 접근 문제로 임시 비활성화
            // devLog('[HomeScreen] useFocusEffect 실행, global.homeScreenRefresh:', global.homeScreenRefresh);

            // // 차단/차단 해제 이벤트 우선 처리
            // if (global.homeScreenRefresh?.userBlocked || global.homeScreenRefresh?.userUnblocked) {
            //     const eventType = global.homeScreenRefresh.userBlocked ? '사용자 차단' : '사용자 차단 해제';
            //     devLog(`🔄 ${eventType} 감지 - 게시물 강제 새로고침`);

            //     // 게시물 목록 강제 새로고침 (차단된 사용자의 게시물 필터링)
            //     setTimeout(() => {
            //         loadPosts(true); // 강제 새로고침 (캐시 버스팅)
            //         loadMyRecentPosts(); // 내 최근 게시물 새로고침
            //     }, 100);

            //     // global.homeScreenRefresh = null;
            //     return;
            // }

            // // 글로벌 이벤트 체크 (WriteMyDayScreen, ProfileEditScreen에서 설정)
            // if (global.homeScreenRefresh?.newPostCreated || global.homeScreenRefresh?.postUpdated || global.homeScreenRefresh?.profileImageUpdated || global.homeScreenRefresh?.profileImageDeleted) {
            //     const eventType = global.homeScreenRefresh.newPostCreated ? '새 글 작성' : global.homeScreenRefresh.postUpdated ? '글 수정' : global.homeScreenRefresh.profileImageDeleted ? '프로필 이미지 삭제' : '프로필 이미지 업데이트';

            //     // 프로필 이미지 업데이트/삭제인 경우 게시물만 새로고침
            //     if (global.homeScreenRefresh.profileImageUpdated || global.homeScreenRefresh.profileImageDeleted) {
            //         devLog(`🔄 ${eventType} 감지 - 게시물 강제 새로고침`);

            //         // 프로필 이미지 삭제의 경우 추가 캐시 초기화
            //         if (global.homeScreenRefresh.profileImageDeleted) {
            //             devLog('🗑️ 프로필 이미지 삭제 - 전체 데이터 초기화');
            //             // 게시물 상태 초기화
            //             setPosts([]);
            //             setMyRecentPosts([]);
            //             lastLoadTime.current = 0; // 캐시 타이머 초기화
            //         }

            //         setTimeout(() => {
            //             loadPosts(true); // 강제 새로고침 (캐시 버스팅)
            //             loadMyRecentPosts(); // 내 최근 게시물 새로고침
            //             loadProfile(); // 프로필 정보도 재로드
            //         }, 100);
            //         // global.homeScreenRefresh = null;
            //         return;
            //     }

            //     // 새 글 작성인 경우에만 오늘 글 상태 업데이트
            //     if (global.homeScreenRefresh.newPostCreated) {
            //         const tempPost = {
            ...
            //         setTodayPost(tempPost);

            //         // AsyncStorage도 즉시 업데이트
            //         const today = new Date().toISOString().split('T')[0];
            //         AsyncStorage.setItem('hasPostedToday', 'true');
            //         AsyncStorage.setItem('todayPostDate', today);
            //         AsyncStorage.setItem('todayPostId', tempPost.post_id.toString());
            ...
            //     // 모든 데이터를 새로고침
            //     setTimeout(() => {
            //         loadPosts(true); // 강제 새로고침
            //         loadMyRecentPosts(); // 내 최근 게시물 새로고침 (감정 아바타 포함)

            //         // 새 글 작성일 때만 오늘 글 상태 재확인
            //         if (global.homeScreenRefresh && global.homeScreenRefresh.newPostCreated) {
...
선택된 이미지 {imageUrls.length}장{imageUrls.length > 1 ? ' (첫 번째만 게시물에 표시)' : ''}
    ...
    // React Query (usePostsQuery)로 대체되었습니다.
    // 캐싱, 성능 최적화는 React Query가 자동으로 처리합니다.
    // 백업용으로 남겨둠 - 필요시 제거 가능
    ...
    const loadPosts = async (forceRefresh: boolean = false) => {
        try {
            ...
            // 캐시 유효성 검사 (5초 이내 재요청 방지 - 서버 부하 감소)
            ...
            // 일반 게시물과 MyDay 게시물을 동시에 조회
            // 강제 새로고침 시 캐시 버스팅을 위해 타임스탬프 추가
            const timestamp = forceRefresh ? Date.now() : undefined;

            // 로그인한 사용자는 북마크 목록도 함께 가져오기
            const promises: Promise<any>[] = [
                postService.getPosts({
                    page: 1,
                    limit: 15,
                    sort_by: 'latest',
                    ...(timestamp && { _t: timestamp }) // 캐시 방지용 타임스탬프
                }),
                myDayService.getPosts({
                    ...
                    return { data: { posts: [] } }; // 실패 시 빈 배열 반환
                })
            ];

            if (isAuthenticated) {
                promises.push(
                    bookmarkService.getBookmarks({ postType: 'my_day' }).catch(error => {
                        devLog('❌ 북마크 목록 로드 실패:', error);
                        return { status: 'error', data: { bookmarks: [] } };
                    })
                );
            }
            ...
            // 중복 제거: 같은 post_id를 가진 게시물이 있는지 확인
            ...
            // MyDay 게시물을 일반 게시물 형식으로 변환 (중복 제거된 것만)
            const convertedMyDayPosts = await Promise.all(uniqueMyDayPosts.map(async (myDayPost) => {
                // MyDay 게시물의 경우 서버 카운트 사용 (DB에서 초기화했으므로 정확함)
                // 별도 API 호출로 인한 성능 저하 및 호환성 문제 방지
                const actualMyDayCommentCount = myDayPost.comment_count || 0;
            ...
            // 두 타입의 게시물을 합치고 시간순으로 정렬
            const allApiPosts = [...apiPosts.map(post => ({ ...post, authorName: post.is_anonymous ? '익명' : (post.user?.nickname || '사용자') })), ...convertedMyDayPosts].sort((a, b) => {
                const dateA = new Date(a.created_at || 0).getTime();
                const dateB = new Date(b.created_at || 0).getTime();
                return dateB - dateA; // 최신순
            });
            // API 데이터를 DisplayPost 형식으로 변환
            const displayPosts: (DisplayPost | null)[] = await Promise.all(
                allApiPosts.map(async (apiPost): Promise<DisplayPost | null> => {
                    try {
                        // 각 게시물에 대한 댓글 로드 - 항상 별도 API 호출로 모든 댓글 로드
                        let comments: any[] = [];
                        devLog(`게시물 ${apiPost.post_id}의 댓글 로드 시작 (강제 API 호출)`);
                        
                        // 내장된 댓글 무시하고 항상 별도 API 호출
                        {
                            try {
                        ...
                        // 댓글 갯수 동기화: 서버 데이터와 실제 댓글 배열 비교
                        const serverCommentCount = apiPost.comment_count || 0;
                        const actualCommentCount = processedComments.length;
                        
                        // 디버깅을 위한 상세 로그
                        // 서버 데이터를 우선하되, 로그로 불일치 확인
                        if (serverCommentCount !== actualCommentCount) {
                        }
                        
                        // 답글이 있는 댓글들을 자동으로 확장 상태에 추가
                        const commentsWithReplies = processedComments
                        ...
                        return {
                            ...
                            authorName: apiPost.is_anonymous ? '익명' : (apiPost.user?.nickname || (apiPost as any).User?.nickname || '사용자'),
                            content: apiPost.content,
                            emotions: apiPost.emotions?.map(emotion => ({
                                emotion_id: emotion.emotion_id,
                                name: emotion.name,
                                icon: emotion.icon,
                                color: localEmotions.find(local => local.label === emotion.name)?.color || '#6366f1'
                            })) || [],
                            image_url: apiPost.image_url ? sanitizeUrl(apiPost.image_url) : undefined,
                            like_count: apiPost.like_count || 0,
                            // 실제 댓글 수를 사용하여 DB와 자동 동기화
                            comment_count: actualCommentCount,
                            created_at: apiPost.created_at || (apiPost as any).createdAt || new Date().toISOString(),
                            updated_at: apiPost.updated_at || (apiPost as any).updatedAt || apiPost.created_at || (apiPost as any).createdAt || new Date().toISOString(),
                            is_anonymous: apiPost.is_anonymous || false,
                            user_id: apiPost.user_id,
                            isLiked: false, // TODO: 좋아요 상태 확인 API 추가 예정
            ...
            // null 값 제거
            const validPosts = displayPosts.filter(post => post !== null) as DisplayPost[];
            ...
            if (isAuthenticated) {
                ...
                devLog('🛡️ [HomeScreen] 클라이언트 차단 필터:', {
                    blockedPosts: Array.from(blockedPostIds),
                    blockedComments: Array.from(blockedCommentIds),
                ...
                const filteredPosts = validPosts.filter(post => {
                    // 차단된 게시물 제외
                    if (blockedPostIds.has(post.post_id)) {
                        devLog(`🚫 [HomeScreen] 차단된 게시물 필터링: ${post.post_id}`);
                        return false;
                    }

                    // 차단된 사용자의 게시물 제외
                    if (blockedUserIds.has(post.user_id)) {
                        devLog(`🚫 [HomeScreen] 차단된 사용자의 게시물 필터링: ${post.post_id} (user: ${post.user_id})`);
                        return false;
                    }
                    ...
                    const filteredComments = post.comments.filter(comment => {
                        // 차단된 댓글 제외
                        if (blockedCommentIds.has(comment.comment_id)) {
                            return false;
                        }

                        // 차단된 사용자의 댓글 제외
                        if (blockedUserIds.has(comment.user_id)) {
                            return false;
                        }
                ...
                if (uniqueFilteredPosts.length !== filteredPosts.length) {
                    devLog(`⚠️ [HomeScreen] 중복 게시물 제거: ${filteredPosts.length} -> ${uniqueFilteredPosts.length}`);
                }

                setPosts(uniqueFilteredPosts);
                devLog(`✅ [HomeScreen] 필터링된 게시물 수: ${uniqueFilteredPosts.length}/${validPosts.length}`);
                // 🔍 데이터 구조 디버깅
                if (uniqueFilteredPosts.length > 0) {
                    const sample = uniqueFilteredPosts[0];
                    devLog('🔍 [HomeScreen] 첫번째 게시물 샘플:', {
                        post_id: sample.post_id,
                        has_emotions: !!sample.emotions,
                        emotions_type: typeof sample.emotions,
                        emotions_is_array: Array.isArray(sample.emotions),
                        emotions_length: sample.emotions?.length,
                        emotions_sample: sample.emotions?.[0] ? {
                            emotion_id: sample.emotions[0].emotion_id,
                            name: sample.emotions[0].name,
                            name_type: typeof sample.emotions[0].name
                        } : 'no emotions'
                    });
                }
            } catch (filterError) {
                devLog('⚠️ [HomeScreen] 클라이언트 필터링 실패, 원본 데이터 사용:', filterError);
                ...
                setPosts(uniquePosts);
                devLog(`✅ [HomeScreen] 비로그인 사용자 - 게시물 ${uniquePosts.length}개 표시`);
            }
