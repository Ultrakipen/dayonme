...
const HomeScreen: React.FC<HomeScreenProps> = () => {
    ...
    const [activeTab, setActiveTab] = useState<FeedTab>('타임라인');
    ...
    const tabFilteredPosts = useTabBasedFeed(filteredPosts, activeTab, emotionPrefs);
...
선택된 이미지 {imageUrls.length}장{imageUrls.length > 1 ? ' (첫 번째만 게시물에 표시)' : ''}
    ...
    const handleReaction = useCallback(async (postId: number, reactionType: ReactionType) => {
        ...
        try {
            // TODO: 백엔드 리액션 API 연동
            // const response = await reactionService.toggleReaction({ post_id: postId, user_id: user.user_id, reaction_type: reactionType });
            ...
            <FlatList
                ...
                {(isAuthenticated || user) ? (
                    <>
                        ...
                        <VStack style={{ paddingHorizontal: normalizeSpace(16), paddingVertical: normalizeSpace(8), gap: normalizeSpace(8) }}>
                            {/* 1행: 프로필 사진 + 환영 인사말 + 아이콘들 */}
                            <HStack style={{ alignItems: 'center', gap: normalizeSpace(10) }}>
                                {/* 프로필 사진 (메모이제이션) */}
                                ...
                                {/* 우측 아이콘들 */}
                                <HStack style={{ gap: normalizeSpace(4) }}>
                                    {/* 프로필/로그인 버튼 */}
                                    <Pressable
                                        onPress={() => {
                                            if (!isAuthenticated) {
                ...
                {!isAuthenticated && (
                    ...
                    >
                        <HStack style={{ alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: normalizeSpace(16), paddingVertical: normalizeSpace(8) }}>
                            <Text
                                style={{
                                    color: isDark ? '#ffffff' : colors.text,
                                    fontSize: normalize(14, 12, 16),
                ...
                {isAuthenticated && weeklyEmotions && weeklyEmotions.length > 0 && (
                    ...
                    >
                        <HStack style={{ justifyContent: 'space-between', alignItems: 'center', marginBottom: isEmotionSectionCollapsed ? 0 : 8 }}>
                            ...
                            >
                                <HStack style={{ alignItems: 'center', gap: 6 }}>
                                    <Text style={{ fontSize: normalize(16, 14, 18) }}>📊</Text>
                                    <Text style={{
                                        fontSize: normalize(14, 12, 16),
                                        fontFamily: 'Pretendard-Bold',
                                        color: colors.text,
                        ...
                        {!isEmotionSectionCollapsed && (
                            <VStack style={{ gap: 6 }}>
                                {/* 요일별 감정 아이콘 */}
                                <HStack style={{ justifyContent: 'space-around', paddingVertical: 3 }}>
                                    {(() => {
                                        const today = new Date();
                                        const currentDay = today.getDay();
                                        const mondayOffset = currentDay === 0 ? -6 : -(currentDay - 1);
                                        ...
                                        return weekDays.map((day, index) => {
                                            ...
                                            return (
                                                <VStack key={day} style={{ alignItems: 'center', gap: 2 }}>
                                                    <Text style={{
                                                    ...
                                                    <Text style={{ fontSize: normalize(20, 18, 22) }}>
                                ...
                                <Box style={{
                                    ...
                                    <Text style={{
                                        ...
                                        이번 주 {weeklyEmotions?.filter(data => data?.emotions && data.emotions.length > 0).length || 0}일 기록! 💪
                                    </Text>
                                </Box>
                ...
                <HStack ref={postsStartRef} className="justify-between items-center" style={{ paddingHorizontal: 8, marginBottom: 2, marginTop: 2 }}>
                ...
                <View style={{
                    ...
                    backgroundColor: colors.cardBackground,
                    borderRadius: normalizeBorderRadius(12),
                    ...
                    {(['타임라인', '맞춤', '인기'] as FeedTab[]).map((tab) => (
                        <Pressable
                            ...
                            style={{
                                ...
                                borderRadius: normalizeBorderRadius(8),
