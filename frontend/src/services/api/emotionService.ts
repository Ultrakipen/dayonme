...
const emotionService = {
  // 모든 감정 목록 조회 (백엔드 /api/emotions)
  getAllEmotions: async () => {
    try {
      console.log('🔄 감정 목록 조회 중...');
...
logEmotion: async (emotionId: number, note?: string, logDate?: string) => {
  try {
    console.log('🔄 단일 감정 로그 저장 중:', { emotionId, note });
    ...
    if (response.data.status === 'success') {
      console.log('✅ 단일 감정 로그 저장 성공');
    }
    
    return response;
  } catch (error) {
    console.error('❌ 단일 감정 로그 저장 오류:', error);
    throw error;
  }
},
  ...
  getEmotionTrends: async (params?: { 
    ...
    try {
      console.log('🔄 감정 트렌드 조회 중...', params);
      const response = await apiClient.get('/stats/trends', { params });
      
      if (response.data.status === 'success') {
        console.log('✅ 감정 트렌드 조회 성공');
      }
  ...
  getDailyEmotionCheck: async () => {
    try {
      console.log('🔄 일일 감정 체크 조회 중...');
      const response = await apiClient.get('/emotions/daily-check');
      
      if (response.data.status === 'success') {
        console.log('✅ 일일 감정 체크 조회 성공');
      }
      
      return response;
    } catch (error) {
      console.error('❌ 일일 감정 체크 조회 오류:', error);
      throw error;
    }
  },
  ...
  getEmotionLogs: async (
    ...
    try {
      ...
      if (response.data.status === 'success') {
        console.log(`✅ 감정 로그 조회 성공: ${response.data.data?.length || 0}개`);
      }
  ...
  syncEmotionsWithPosts: async () => {
    try {
      console.log('🔄 감정 기록 동기화 시작...');
      const response = await apiClient.post('/emotions/sync');
      
      if (response.data.status === 'success') {
        console.log('✅ 감정 기록 동기화 성공');
      }
      
      return response;
    } catch (error) {
      console.error('❌ 감정 기록 동기화 오류:', error);
      throw error;
    }
  }
};
